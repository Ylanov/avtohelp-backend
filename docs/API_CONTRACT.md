...
## 12. Что потребует действий уже сейчас

- **Hardcoded IP в Android (`153.80.245.74:8000`)** — при деплое на новый облачный сервер либо этот IP переезжает туда, либо фронт пересобирается с новым URL. Согласовать с пользователем на этапе деплоя.
- **HTTP (не HTTPS)** в dev/prod конфиге фронта — нужно будет включить HTTPS и пересобрать фронт для продакшн-демо.
