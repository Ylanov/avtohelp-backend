...
class TestOrder(APITestCase):
    ...
    def setUp(self):
        ...
        # Create car models
        self.toyota_model = car_models.CarModel.objects.create(
        ...
        # Create devices
        self.device_1 = profile_models.FCMDevice.objects.create(
            user=self.user_1, active=True
        )
        self.device_2 = profile_models.FCMDevice.objects.create(
            user=self.user_2, active=True
        )
        self.device_3 = profile_models.FCMDevice.objects.create(
            user=self.user_3, active=True
        )

        # Create assistance requests
        self.assistance_request_0 = models.AssistanceRequest.objects.create(
            user=self.user_1,
            issue="Issue 1",
            description="Description",
            location=Point(45.061016, 38.944007, srid=4326),
        )
        self.assistance_request_1 = models.AssistanceRequest.objects.create(
            user=self.user_2,
            issue="Issue 2",
            description="Description",
        ...
        # Create user cars
        self.car_1 = car_models.Car.objects.create(
            mark=self.toyota, car_model=self.toyota_model
        )
        self.car_user_1 = profile_models.ProfileCar.objects.create(
            owner=self.user_1,
            car=self.car_1,
            color=self.color_1,
    ...
    def test_service_list_query_2(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data[0].get("profile_id"),
            self.assistance_request_1.user.profile.id,
        )
    ...
    def test_sent_notification_by_distance_and_location_relevance(self):
        ...
        devices = (
            profile_models.FCMDevice.objects.filter(active=True)
    ...
    def test_assistance_request_is_owner_status(self):
        ...
        response = self.client.get(
            reverse(api_path, kwargs={"pk": self.assistance_request_0.pk})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_assistance_request_is_owner_status_2(self):
        ...
        response = self.client.get(
            reverse(api_path, kwargs={"pk": self.assistance_request_1.pk})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
