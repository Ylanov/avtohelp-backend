...
class TestProfile(APITestCase):
    ...
    def test_profiles_list_1(self):
        ...
        self.assertEqual(
            ...
            Profile.objects.exclude(user=self.user_1).count(),
        )
    ...
    def test_profiles_list_3(self):
        ...
        self.assertEqual(
            len(response.data.get("results")),
            FriendList.objects.my_list(user=self.user_1).count(),
        )
    ...
    def test_friend_requests_1(self):
        ...
        self.assertEqual(
            ...
            FriendRequest.objects.not_approved().from_me(self.user_1).count(),
        )
    ...
    def test_friend_request_to_user_4(self):
        ...
        response = self.client.get(
            reverse(api_path), data={"person_id": user_2.profile.id}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data.get("count"),
            FriendRequest.objects.not_approved()
            .to_me(self.user_1)
            .filter(owner=user_2)
            .count(),
        )
    ...
    def test_friend_list(self):
        ...
        FriendList.objects.create(
            owner=user_3, friend=self.user_1, request=friend_request
        )
    ...
    def test_friend_list_3(self):
        ...
        FriendList.objects.create(
            owner=user_2, friend=self.user_1, request=friend_request
        )
    ...
    def test_remove_friend_from_friendlist(self):
        ...
        response = self.client.delete(
            reverse(api_path, kwargs={"profile_id": user_2.profile.id})
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    ...
    def test_profile_cars(self):
        ...
        self.assertEqual(
            response.data.get("count"),
            ProfileCar.objects.filter(owner=self.user_1).count(),
        )
    ...
    def test_profile_car_detail(self):
        ...
        # Create second car
        car_2 = ProfileCar.objects.create(
            owner=self.user_1,
            car=self.car_2,
            color=self.color_1,
            license_plate="xxx123xx 70",
    ...
    def test_profile_car_detail_update(self):
        ...
        self.assertEqual(
            ...
            ProfileCar.objects.get(id=car_2.id).license_plate,
        )
    ...
    def test_profile_car_delete(self):
        ...
        response = self.client.delete(
            reverse(api_path, kwargs={"pk": self.car_user_1.id})
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
    ...
    def test_annotated_field_friend_1(self):
        ...
        api_path = "%s:userprofile:profile-detail" % self.VERSION
        response = self.client.get(
            reverse(api_path, kwargs={"pk": self.user_1.profile.id})
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get("friend"), True)
    ...
    def test_update_profile_location(self):
        ...
        data = {"geo_lat": 45.849324, "geo_lon": 38.960508}
