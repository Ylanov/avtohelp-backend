...
class TestChat(APITestCase):
    ...
    def test_room_detail(self):
        ...
        api_path = "%s:chat:room-detail" % settings.AVAILABLE_VERSIONS.get(
            "current"
        )
    ...
    def test_chat_messages_list(self):
        ...
        ChatMessage.objects.create(
            sender=self.user_1, room=room, message="Hello"
        )

        api_path = "%s:chat:message-list" % settings.AVAILABLE_VERSIONS.get(
            "current"
        )
        response = self.client.get(reverse(api_path, kwargs={"pk": room.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_total_chat_room_messages_unread_count(self):
        ...
        Mechanism:
            from user_1 to user_2 (m_1)
            from user_2 to user_1 (m_2),
        ...
        # Create messages
        ChatMessage.objects.create(sender=self.user_1, room=room, message="Hi")
        m_2 = ChatMessage.objects.create(
            sender=self.user_2, room=room, message="Hello"
        )
        ChatMessage.objects.create(
    ...
    def test_total_count_chat_room_unread_messages_w_blacklisted_user(
        self,
    ):
        """
        ...
        Total messages:
            3 (user_1 to user_2, user_2 to user_1, user_2 to user_1)
        Total messages exclude message from blacklisted user: 1
        Mechanism:
            user_1 write message to user_2 (M1), user_2 answered user_1 (M2),
    ...
    def test_chat_create_error_3(self):
        """
        Test create chat room w/ person who was your friend
        but now in black list
        """
        api_path = (
            "%s:chat:private-room-create"
            % settings.AVAILABLE_VERSIONS.get("current")
    ...
    def test_impossibility_chatting_with_blacklisted_user(self):
        ...
        Now chat room list should return not a single room
        'cause of Blacklist obj
    ...
    def test_possibility_chatting_only_with_friends(self):
        ...
        Now chat room list should return only rooms when
        ...
        self.assertEqual(
            chat_room, ChatRoom.objects.friends(self.user_1).first()
        )
    ...
    def test_chat_room_unread_messages_counter(self):
        ...
        Participants: user_1, user_2
        Mechanism:
            user_1 wrote message to user_2 (m_1)
            user_2 wrote twice to user_1 (m_2 & m_3)
        Correct count of unread messages:
            2 (as user_1), 1 (as user_2)
        """
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data.get("results")[0].get("unread_messages"), 2
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data.get("results")[0].get("unread_messages"), 1
        )
    ...
    def test_chat_messages_annotated_read_status(self):
        ...
        Participants: user_1, user_2
        Mechanism:
            user_1 sent user_2 message (m_1)
            user_2 reply user_1 twice (m_2, m_3)
        """
        # Authorize user_1, that not allowed to read this conversation
        self.token, created = Token.objects.get_or_create(user=self.user_1)
        self.client.credentials(HTTP_AUTHORIZATION="Token " + self.token.key)
        ...
        m_3 = ChatMessage.objects.create(
            sender=self.user_2, room=room, message="sup"
        )
    ...
    def test_chat_room_list_filter_by_participant_id(self):
        ...
        response = self.client.get(
            reverse(api_path),
            data={"participant_id": self.user_1.profile.id},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response.data.get("results")),
    ...
    def test_ordering_chat_rooms_by_last_message(self):
        ...
        ChatMessage.objects.create(
            sender=self.user_1, room=room_1, message="Message"
        )
